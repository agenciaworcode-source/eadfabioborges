...
async function verifyGap3() {
  ...
  try {
    ...
    assert(progress.percentage === 0, '3.4c Initial percentage is 0', `Got: ${progress.percentage}`)
...
async function verifyGap4() {
  ...
  try {
    ...
    const coreOnlyResult = await coreOnlyValidator.validateAgentReferences([{ agent: 'architect' }])
