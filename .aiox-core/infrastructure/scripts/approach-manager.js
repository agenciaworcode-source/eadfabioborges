...
class ApproachManager {
  ...
  constructor(options) {
    ...
    if (options.recoveryPath) {
      ...
      this.recoveryPath = path.join(this.rootPath, 'docs/stories', this.storyId, CONFIG.recoveryDir)
    }
...
`
  ...
  async getSuggestions(subtaskId) {
    ...
    if (failedApproaches.length >= 3) {
      ...
      suggestions.recommendedActions.push('Review story requirements for possible misunderstanding')
