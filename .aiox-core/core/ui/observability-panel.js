...
class ObservabilityPanel {
   ...
   */
  removeTerminal(pid) {
    this.state.active_terminals.list = this.state.active_terminals.list.filter((t) => t.pid !== pid)
    this.state.active_terminals.count = this.state.active_terminals.list.length
  }
   ...
   */
  toggleMode() {
    this.state.mode = this.state.mode === PanelMode.MINIMAL ? PanelMode.DETAILED : PanelMode.MINIMAL
