...
class BuildStateManager {
  ...
  detectAbandoned(threshold = null) {
    ...
    if (elapsed > thresholdMs && state.status === BuildStatus.IN_PROGRESS) {
      ...
      // Add notification
      this._addNotification(NotificationType.ABANDONED, `Build abandoned: ${state.abandonedReason}`)
  ...
  formatStatus() {
    ...
    lines.push(`  Avg Time:  ${this._formatDuration(status.metrics.averageTimePerSubtask)}/subtask`)
