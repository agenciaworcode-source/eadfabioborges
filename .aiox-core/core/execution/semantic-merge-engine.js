  ...
  async mergeFile(filePath, baseContent, taskSnapshots) {
    ...
    for (const [taskId, snapshot] of Object.entries(taskSnapshots)) {
      ...
      if (taskContent !== undefined) {
        taskAnalyses[taskId] = this.analyzer.analyzeDiff(filePath, baseContent, taskContent, taskId)
    ...
    for (const conflict of conflicts) {
      ...
      if (autoResult.success) {
        ...
        const aiResult = await this.aiResolver.resolveConflict(conflict, baseContent, taskSnapshots)
        results.push({
          conflict,
          result: aiResult,
