...
export async function GET(_req: Request, { params }: { params: { id: string } }) {
  ...
  const { data, error: dbError } = await supabase!
    .from('lessons')
    .select(
      'id, module_id, title, type, vimeo_id, content_body, embed_url, pdf_url, duration_secs, order, is_free_preview'
    )
    .eq('id', params.id)
    .single()
...
export async function DELETE(_req: Request, { params }: { params: { id: string } }) {
  ...
  const { error: dbError } = await supabase!.from('lessons').delete().eq('id', params.id)
